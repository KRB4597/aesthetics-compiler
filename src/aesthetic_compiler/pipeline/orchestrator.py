...
from aesthetic_compiler.ir.graph.promote import graph_from_ir
from aesthetic_compiler.ir.graph.canonical import canonical_hash
from aesthetic_compiler.am_dag.dag import AMDAG
from aesthetic_compiler.audit.hash_chain import build_audit
from aesthetic_compiler.tiers import CompilerTier, InputMode, ALL_PROJECTIONS, detect_input_mode
...
def compile_document(
    ...
    *,
    ...
    if detected == InputMode.TEXT:
        from aesthetic_compiler.ingestion.text_loader import load_text
        ...
        from aesthetic_compiler.ingestion.image_loader import load_image
        ...
        from aesthetic_compiler.ingestion.html_loader import load_html
        ...
        from aesthetic_compiler.ingestion.multimedia_loader import load_multimedia
    ...
    segments = []
    if detected == InputMode.TEXT:
        from aesthetic_compiler.segmentation.segmenter import segment_text
    ...
    extractor_tier: str
    if detected == InputMode.TEXT:
        from aesthetic_compiler.annotation.rule_extractor import RuleExtractor
        result = RuleExtractor().extract(text)  # type: ignore[name-defined]
        extractor_tier = CompilerTier.RULE
    elif detected == InputMode.IMAGE:
        from aesthetic_compiler.annotation.image_extractor import ImageExtractor
        result = ImageExtractor().extract(raw_meta)
        extractor_tier = CompilerTier.IMAGE
    elif detected == InputMode.HTML:
        from aesthetic_compiler.annotation.html_extractor import HtmlExtractor
        result = HtmlExtractor().extract(raw_meta)
        extractor_tier = CompilerTier.HTML
    elif detected == InputMode.MULTIMEDIA:
        from aesthetic_compiler.annotation.multimedia_extractor import MultimediaExtractor
        result = MultimediaExtractor().extract(frames, raw_meta)
        extractor_tier = CompilerTier.MULTIMEDIA
    else:
        from aesthetic_compiler.annotation.mock_extractor import MockExtractor
    ...
    # ------------------------------------------------------------------
    from aesthetic_compiler.canonicalizer.registry import StyleRegistry
