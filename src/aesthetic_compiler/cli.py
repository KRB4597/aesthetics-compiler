...
from aesthetic_compiler import __version__
from aesthetic_compiler.tiers import ALL_PROJECTIONS, InputMode

app = typer.Typer(help="geometric-aesthetics-compiler — aesthetic IR with pluggable projection analysers")
console = Console()
...
def export_schema_org_cmd(
    ...
    from aesthetic_compiler.ir.schemas import AestheticIR
    from aesthetic_compiler.export.schema_org import export_schema_org
    ir = AestheticIR.model_validate_json(ir_file.read_text(encoding="utf-8"))
...
def version() -> None:
    ...
    console.print(f"geometric-aesthetics-compiler v{__version__}")
...
def _print_summary(ir) -> None:
    ...
    from aesthetic_compiler.ir.schemas import AESTHETIC_DIMENSIONS
